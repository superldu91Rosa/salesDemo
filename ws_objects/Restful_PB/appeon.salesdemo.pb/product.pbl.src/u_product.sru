...
Choose Case adw_data.dataobject
	Case "d_subcategory"
		...
		gnv_restclient.RetrieveWithModel(adw_data, gs_host + "/RetrieveWithModel", Integer(as_data) )
		IF adw_data.RowCount() > 0 Then
			il_subcate_id = adw_data.GetItemNumber(1, "productsubcategoryid")
		End IF	
	Case "d_product"
		//adw_data.Retrieve(Long(as_data))
		gnv_restclient.RetrieveWithModel(adw_data, gs_host + "/RetrieveWithModel", Long(as_data) )
		If adw_data.RowCount() > 0 Then
...
Else
	...
	//ldwc_cate.Retrieve( )
	gnv_restclient.RetrieveWithModel(ldwc_cate, gs_host + "/RetrieveWithModel" )
	
	dw_cate.SetItem(1, "id", il_cate_id)
End IF

//tab_1.tabpage_1.dw_browser.Retrieve( il_cate_id )
ll_Return = gnv_restclient.RetrieveWithModel(tab_1.tabpage_1.dw_browser, gs_host + "/RetrieveWithModel", il_cate_id )
If ll_Return = -1 Then
...
//					AND production.product.productsubcategoryid = :ll_subcateid);
			lds_data.dataObject = "d_productphoto2"
			gnv_restclient.RetrieveWithModel(lds_data, gs_host + "/RetrieveWithModel", ll_subcateid )
			For ll_i = lds_data.Rowcount() To 1 Step -1
				lds_data.DeleteRow(ll_i)
			Next
			If gnv_restclient.UpdateWithModel(lds_data, gs_host + "/UpdateWithModel") <> 1 Then Return -1
...
//			AND  production.product.productsubcategoryid =  :ll_subcateid; 
			...
			lds_data.dataObject = "d_history_price2"
			gnv_restclient.RetrieveWithModel(lds_data, gs_host + "/RetrieveWithModel", ll_subcateid )
			For ll_i = lds_data.Rowcount() To 1 Step -1
				lds_data.DeleteRow(ll_i)
			Next
...
//			END IF
			...
			If gnv_restclient.UpdateWithModel(tab_1.tabpage_1.dw_browser, gs_host + "/UpdateWithModel") <> 1 Then Return -1
...
//			END IF
	...
	Case "dw_productlist", "dw_master"
		...
		IF li_ret = 1 Then
			...
			lds_data.dataObject = "d_history_price"
			gnv_restclient.RetrieveWithModel(lds_data, gs_host + "/RetrieveWithModel", ll_productid )
			For ll_i = lds_data.Rowcount() To 1 Step -1
				lds_data.DeleteRow(ll_i)
			Next
			If gnv_restclient.UpdateWithModel(lds_data, gs_host + "/UpdateWithModel") <> 1 Then Return -1
...
//			END IF
			...
			lds_data.dataObject = "d_productphoto"
			gnv_restclient.RetrieveWithModel(lds_data, gs_host + "/RetrieveWithModel", ll_productid )
			For ll_i = lds_data.Rowcount() To 1 Step -1
				lds_data.DeleteRow(ll_i)
			Next
...
//	Return -1
//END IF
if gnv_restclient.UpdateWithModel(iuo_currentdw, gs_host + "/UpdateWithModel") <> 1 Then
	IF ldws_1 = NewModified! OR ldws_1 = DataModified! Then
         tab_1.tabpage_2.dw_detail.DeleteRow( li_prow )
    END IF
	 Return -1
...
IF ls_dwname =  "dw_master" and ldws_sign = NewModified! Then
	...
	ll_pkid = tab_1.tabpage_1.dw_productlist.GetItemNumber( tab_1.tabpage_1.dw_productlist.GetRow(), "productid" )
...
////lnv_objects.setadditionaldata("SELECT Max(Isnull(ProductPhotoID, 0)) + 1 FROM Production.ProductPhoto")
////gnv_restclient.retrieveorupdatewithmodels (lnv_objects, gs_host + "/RetrieveOrUpdateWithModels" )

////MessageBox("",lnv_objects.GetSQLDBCodeAt(1))
