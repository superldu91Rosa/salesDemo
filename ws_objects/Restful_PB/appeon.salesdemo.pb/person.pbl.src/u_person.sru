...
public function integer of_winopen ();Long ll_Return
This.SetRedraw(False)
//Retrieve Dddw
//open(w_progressbar)
//il_start = cpu()
//Tab_1.tabpage_1.dw_browser.function post Retrieve(is_persontype)
ll_return = gnv_restclient.RetrieveWithModel(Tab_1.tabpage_1.dw_browser, gs_host + "/RetrieveWithModel", is_persontype )
If ll_Return = -1 Then
...
Long ll_businessentityid, ll_Return
integer ll_ret
...
//		ll_ret = lnv_objects.addretrieveobject(tab_1.tabpage_1.dw_persondetail, ll_businessentityid)
//		ll_ret = lnv_objects.addretrieveobject(tab_1.tabpage_2.dw_master, ll_businessentityid)
//		ll_ret = lnv_objects.addretrieveobject(tab_1.tabpage_2.dw_detail, ll_businessentityid)
//		ll_ret = lnv_objects.addretrieveobject(tab_1.tabpage_2.dw_cust, ll_businessentityid)
		
//		ll_ret = gnv_restclient.retrieveorupdatewithmodels (lnv_objects, gs_host + "/RetrieveOrUpdateWithModels" )
//		If ll_ret = -1 Then
//			MessageBox(String(gnv_restclient.getresponsestatuscode()), gnv_restclient.getresponsestatustext())
//		End If
		
		ll_Return = gnv_restclient.Retrievewithmodel(tab_1.tabpage_1.dw_persondetail, gs_host + "/RetrieveWithModel", ll_businessentityid)
		If ll_Return = -1 Then
			MessageBox(String(gnv_restclient.getresponsestatuscode()), gnv_restclient.getresponsestatustext())
		End If
		
		gnv_restclient.Retrievewithmodel(tab_1.tabpage_2.dw_master, gs_host + "/RetrieveWithModel", ll_businessentityid)
		gnv_restclient.Retrievewithmodel(tab_1.tabpage_2.dw_detail, gs_host + "/RetrieveWithModel", ll_businessentityid)
		gnv_restclient.Retrievewithmodel(tab_1.tabpage_2.dw_cust, gs_host + "/RetrieveWithModel", ll_businessentityid)
...
Choose case iuo_currentdw.ClassName()
	Case	"dw_browser", "dw_persondetail"		
		...
		IF li_ret = 1 Then
			...
			lds_data.dataobject = "d_businessentityaddress"
			gnv_restclient.retrievewithmodel(lds_data, gs_host + "/RetrieveWithModel", ll_personid)
			if lds_data.Rowcount() > 0 Then
				lds_data.deleterow(1)
			End If
			If gnv_restclient.updatewithmodel(lds_data, gs_host + "/RetrieveWithModel") <> 1 Then
				Return -1
			End If
...
//			END IF
			...
			lds_data.dataobject = "d_personphone"
			gnv_restclient.retrievewithmodel(lds_data, gs_host + "/RetrieveWithModel", ll_personid)
			if lds_data.Rowcount() > 0 Then
				lds_data.deleterow(1)
			End If
...
If tab_1.tabpage_1.dw_persondetail.Modifiedcount() > 0 Then
	...
	IF ldws_status = Newmodified! Then
		...
		lds_data.dataObject = "d_businessentity"
		
		ll_insert = lds_data.Insertrow(0)
		lds_data.SetItem(ll_insert, "ModifiedDate", ldt_now)
		If gnv_restclient.UpdateWithModel(lds_data, gs_host + "/UpdateWithModel") <> 1 Then
...
//			Return -1
//		END IF
		ll_pk = lds_data.GetItemnumber(ll_insert, "BusinessEntityID")
...
//		END IF
	...
	li_status =  gnv_restclient.UpdateWithModel(tab_1.tabpage_1.dw_persondetail, gs_host + "/UpdateWithModel")
...
//			Return -1
//		END IF
		If gnv_restclient.UpdateWithModel(tab_1.tabpage_1.dw_persondetail, gs_host + "/UpdateWithModel") <> 1 Then
			tab_1.tabpage_1.dw_browser.SetRedraw(True)
			Return -1 
		End If
	END IF
...
ElseIf tab_1.tabpage_2.dw_master.Modifiedcount()  + tab_1.tabpage_2.dw_detail.Modifiedcount()  + &
		...
		If tab_1.tabpage_2.dw_master.Modifiedcount() > 0 Then
			...
			li_status = gnv_restclient.UpdateWithModel(tab_1.tabpage_2.dw_master, gs_host + "/UpdateWithModel")
		...
		If tab_1.tabpage_2.dw_detail.Modifiedcount() > 0 and Not lb_error Then
			...
			li_status = gnv_restclient.UpdateWithModel(tab_1.tabpage_2.dw_detail, gs_host + "/UpdateWithModel")
			If li_status = -1 Then lb_error = True
		End If
		
		If tab_1.tabpage_2.dw_cust.Modifiedcount() > 0 and Not lb_error Then
			//li_status = tab_1.tabpage_2.dw_cust.update( )
			li_status = gnv_restclient.UpdateWithModel(tab_1.tabpage_2.dw_cust, gs_host + "/UpdateWithModel")
			If li_status = -1 Then lb_error = True
		End If
